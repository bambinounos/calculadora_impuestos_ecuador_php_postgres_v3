...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAuthUI() {
        ...
        if (currentUser) {
            ...
            tariffManagementSection.style.display = isAdmin ? 'block' : 'none'; // Mostrar solo a admin
    ...
    async function handleLogout() {
        try {
            ...
            currentUser = null;
            updateAuthUI();
            // Limpiar datos sensibles o específicos del usuario al hacer logout
    ...
    async function loadDefaultTariffCodes() { 
        try {
            ...
            const data = await response.json();
            tariffCodeIdSelect.innerHTML = '<option value="">Seleccione una partida</option>'; // Limpiar antes de añadir
    ...
    async function loadSavedCalculations() {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}calculations.php?action=load`);
            const data = await response.json();
            if (data.success) { renderSavedCalculations(data.calculations); } 
    ...
    function renderSavedCalculations(calculations) {
        ...
        if (calculations.length === 0) {
            savedCalculationsListDiv.innerHTML = '<p>No tienes cálculos guardados.</p>'; return;
        }
        ...
        calculations.forEach(calc => {
            ...
            li.innerHTML = `
                <div class="calc-info">
                    ...
                    <small>Guardado: ${new Date(calc.created_at).toLocaleDateString()}</small>
                </div>
                <div class="actions">
        ...
        try {
            ...
            if (data.success || (data.items_processed_details && data.items_processed_details.length > 0)) {
                ...
                if (data.consolidated_results) {
                    ...
                    csvResultsSummaryDiv.innerHTML = \`
                        <div class="section-title">Resumen Consolidado de Importación CSV</div>
                        <table>
    ...
    tariffCodeForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                loadTariffCodesForManagement(); // Recargar lista
                loadDefaultTariffCodes(); // Recargar select en calculadora principal
            }
    ...
    async function loadTariffCodesForManagement() {
        try {
            const response = await fetch(`${API_BASE_URL}tariff_codes.php?action=read`); // Podría paginarse
            const data = await response.json();
    ...
    // --- Inicialización ---
    checkLoginStatus(); // Verifica si hay sesión activa y actualiza UI
    loadDefaultTariffCodes(); // Carga partidas en el select de la calculadora principal
});
